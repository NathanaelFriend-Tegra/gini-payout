...
Things to Watch
SituationWhat to doResponse field doesn't match your interfaceconsole.log the raw response and update the interface.  
 400 Bad RequestLog req.body in the proxy — a required field is likely missing or wrong format  
 500 from OmneaTheir server error — share the apimStatus.marketplaceId with
  Omnea supportMobile numberAlways strip +27 and replace with 0 before sendingEmpty emailUse user.email || 'noreply@ginipayout.co.za' — Omnea rejects empty strings  
